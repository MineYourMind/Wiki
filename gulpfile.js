...
function watch() {
  gulp.watch('./docs/source/**', gulp.series('sphinx:dev'));
};

gulp.task('clean', () => del(['build']));

gulp.task('sphinx', shell(
    'sphinx', 'sphinx-build', ['-W', '-d', 'build/doctrees', 'docs/source', 'build/html']
));

gulp.task('sphinx:dev', shell(
    'sphinx', 'sphinx-build', ['docs/source', 'build/dev/html']
));
